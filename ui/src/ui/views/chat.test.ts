...
describe("chat view", () => {
  ...
  it("keeps live tool cards chronologically before final assistant message", () => {
    const container = document.createElement("div");
    render(
      renderChat(
        createProps({
    ...
    expect(text.indexOf("tool output")).toBeGreaterThanOrEqual(0);
    expect(text.indexOf("final answer")).toBeGreaterThanOrEqual(0);
    expect(text.indexOf("tool output")).toBeLessThan(text.indexOf("final answer"));
  });

  it("deduplicates tool cards when tool events exist in both transcript and live stream", () => {
    const container = document.createElement("div");
    render(
      renderChat(
        createProps({
          ...
          messages: [
            {
              ...
              toolCallId: "tool-dup",
              timestamp: 200,
              content: [{ type: "tool_result", text: "duplicate tool" }],
            },
          ],
          toolMessages: [
            {
    ...
    const text = container.textContent ?? "";
    const matches = text.match(/duplicate tool/g) ?? [];
