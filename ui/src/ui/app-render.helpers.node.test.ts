...
describe("resolvePreferredSessionKeyForAgent", () => {
  it("returns current key when already in selected agent", () => {
    const sessions: SessionsListResult = {
      ...
      sessions: [
        row({ key: "agent:main:main", updatedAt: 100 }),
        row({ key: "agent:coder:main", updatedAt: 200 }),
      ],
    };
    expect(
      resolvePreferredSessionKeyForAgent(
    ...
    ).toBe("agent:coder:main");
  });

  it("picks most recently active session for selected agent", () => {
    const sessions: SessionsListResult = {
      ts: 0,
      path: "",
      count: 3,
      defaults: { model: null, contextTokens: null },
      sessions: [
        row({ key: "agent:coder:main", updatedAt: 100 }),
        row({ key: "agent:coder:telegram:direct:alice", updatedAt: 500 }),
        row({ key: "agent:main:main", updatedAt: 300 }),
      ],
    };
    expect(
      resolvePreferredSessionKeyForAgent("coder", "agent:main:main", sessions, "agent:main:main"),
    ).toBe("agent:coder:telegram:direct:alice");
  });

  it("falls back to agent main session when no session exists for selected agent", () => {
    const sessions: SessionsListResult = {
      ts: 0,
      path: "",
      count: 1,
      defaults: { model: null, contextTokens: null },
      sessions: [row({ key: "agent:main:main", updatedAt: 100 })],
    };
    expect(
      resolvePreferredSessionKeyForAgent("coder", "agent:main:main", sessions, "agent:main:main"),
    ).toBe("agent:coder:main");
